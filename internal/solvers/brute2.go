...
func makeSolutionFromSubsets2(ins instance, subsetIndices []int, best subsetsEval) subsetsEval {
...
func SolveByBruteForce2(ins instance) (subsetsEval, error) {
	...
	bestSubsetsEval := makeSolutionFromSubsets2(ins, nil, subsetsEval{})
	...
	for i := 1; i <= nSubsetsToTry; i++ {
		...
		for combinations.Next() {
			...
			subsetEval := makeSolutionFromSubsets2(ins, perm, bestSubsetsEval)
