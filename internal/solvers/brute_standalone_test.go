...
func TestInfeasible2(t *testing.T) {
	...
	result, err := SolveByBruteForceInternal2(ins)
	assert.NilError(t, err)
	assert.Assert(t, !result.ExactlyCovered, "should be infeasible")
}

func TestTooBigForBrute2(t *testing.T) {
	...
	assert.NilError(t, err)
	_, err = SolveByBruteForceInternal2(ins)
...
func testBruteFindsEquallyGoodSolution2(t *testing.T, spec cover.TestInstanceSpecification) {
...
func TestBruteOnTinyInstances2(t *testing.T) {
	t.Parallel()
	...
	for _, spec := range instanceSpecifications {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			testBruteFindsEquallyGoodSolution2(t, spec)
		})

	}
}

func BenchmarkBruteOnRandomTinyInstances2(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for j := 0; j < len(instances); j++ {
			_, err := SolveByBruteForceInternal2(instances[j])
